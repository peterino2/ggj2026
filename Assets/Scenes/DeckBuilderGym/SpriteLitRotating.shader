...
// Copypasted from the builtin sprite-lit-default
Shader "Custom/SpriteLitGlow"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // -------------------------------------
            // Shader Stages
            #pragma vertex RotatingVertSimple
            #pragma fragment RotatingFragSimple
            ...
            Varyings RotatingVertSimple(Attributes input)
            {
                return LitPassVertexSimple(input);
            }

            void RotatingFragSimple(
                Varyings input
            ...
            )
            {
                ...
                // Run the standard SimpleLit fragment
                #ifdef _WRITE_RENDERING_LAYERS
                LitPassFragmentSimple(input, outColor, outRenderingLayers);
                #else
                LitPassFragmentSimple(input, outColor);
                #endif

                // Pulsate emission
                float pulse = lerp(_EmissionPulseMin, _EmissionPulseMax, (sin(_Time.y * _EmissionPulseSpeed * 3) * 0.2 + 0.5));
                #ifdef _EMISSION
                half4 emissionSample = SAMPLE_TEXTURE2D(_EmissionMap, sampler_EmissionMap, input.uv);
                half3 emissionBase = emissionSample.rgb * _EmissionColor.rgb;
                #else
                half3 emissionBase = _EmissionColor.rgb;
                #endif
                outColor.rgb += emissionBase * (pulse - 1.0);
            }
            ENDHLSL
        }
